...
def xmodem_send_bin():
    ...
    while (idx < len(model_list)):
        ...
        print(colorama.Fore.BLUE + colorama.Style.BRIGHT + "xmodem_sending >>", model_file, colorama.Style.RESET_ALL)
...
if __name__ == '__main__':
    ...
    ret = xmodem_send_bin()
    if (ret) :
        print(colorama.Fore.GREEN + colorama.Style.BRIGHT + "xmodem_send bin file done!!")
    else :
        print(colorama.Fore.RED + colorama.Style.BRIGHT + "xmodem_send bin file FAIL!!!!")
    ...
    ###    response = ser.readline().strip()
    ###    print(str(response))
    ###    if need_reset and (str(response).count('Do you want to end file transmission and reboot system? (y)') > 0) :
    ###        time.sleep(1)
    ###        ser.flushInput()
    ###        send_at_command('y')
    ###        need_reset = False
